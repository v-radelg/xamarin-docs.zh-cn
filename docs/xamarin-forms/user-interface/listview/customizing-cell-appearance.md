...
[`ListView`](xref:Xamarin.Forms.ListView)显示可滚动列表, 可通过使用`ViewCell`对其进行自定义。 `ViewCells` 可用于显示文本和图像、 指示 true/false 状态和接收用户输入。
...
- 在中`ViewCell`, 布局可以由任意 Xamarin 布局进行管理。 

...
description: 本文说明如何创建单页跨平台 Xamarin.Forms 应用程序，通过该应用程序可输入便笺并将其保存到设备存储。
...
本快速入门演练如何创建跨平台 Xamarin.Forms 应用程序，通过该应用程序可输入便笺并将其保存到设备存储。 最终的应用程序如下所示：
...
6. 在“MainPage.xaml”中，删除所有模板代码，替换为以下代码  ：
    ...
    该代码以声明的方式定义页面的用户界面，它包含一个用于显示文本的 [`Label`](xref:Xamarin.Forms.Label)、一个用于文本输入的 [`Editor`](xref:Xamarin.Forms.Editor) 以及两个指示应用程序保存或删除文件的 [`Button`](xref:Xamarin.Forms.Button) 实例。 这两个 `Button` 实例水平放置在 [`Grid`](xref:Xamarin.Forms.Grid) 中，而 `Label`、`Editor` 和 `Grid` 垂直放置在 [`StackLayout`](xref:Xamarin.Forms.StackLayout) 中。 有关创建用户界面的详细信息，请参阅 [Xamarin.Forms 快速入门深入探讨](deepdive.md)中的[用户界面](deepdive.md#user-interface)。

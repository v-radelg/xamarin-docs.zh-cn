...
[![](objective-c-libraries-images/00-sml.png "通过选择项目类型 \"iOS 库绑定库\" Visual Studio for Mac 执行此操作")](objective-c-libraries-images/00.png#lightbox)
...
你将在其中定义 API 协定, 这是描述如何将基础目标 C API 投影到C#的文件。 `ApiDefinition.cs` 此文件的语法和内容是本文档讨论的主要主题, 其中的内容仅限于C#接口和C#委托声明。 `StructsAndEnums.cs`文件是您将在其中输入接口和委托所需的任何定义的文件。 这包括你的代码可能使用的枚举值和结构。
...
上面的示例定义了一个名`Cocos2D.Camera`为的类, `NSObject`该类派生自基类型`Foundation.NSObject`(此类型来自), 并定义了一个`ZEye`静态属性 ()、两个不采用参数的方法和一个采用三个方法的方法形参.
...
此图显示了文件之间的关系:
 ...
 [![](objective-c-libraries-images/screen-shot-2012-02-08-at-3.33.07-pm.png "此图显示了文件之间的关系")](objective-c-libraries-images/screen-shot-2012-02-08-at-3.33.07-pm.png#lightbox)

API 定义文件将只包含命名空间和接口定义 (具有接口可以包含的任何成员), 不应包含类、枚举、委托或结构。 API 定义文件只是将用于生成 API 的协定。

枚举或支持类所需的任何额外代码都应托管在单独的文件中, 在上面的示例中, "CameraMode" 是一个枚举值, 该枚举值在 CS 文件中不存在, 应托管在单独的文件中`StructsAndEnums.cs` , 例如:
...
该`APIDefinition.cs` 文件`StructsAndEnum`与类结合使用, 用于生成库的核心绑定。 您可以按原样使用生成的库, 但通常情况下, 您需要优化生成的库, 以便为用户C#的权益添加某些功能。 一些示例包括实现`ToString()`方法、提供C#索引器、向和从某些本机类型添加隐式转换或提供某些方法的强类型版本。 这些改进存储在额外C#文件中。 只需将C#文件添加到项目, 这些文件就会包含在此生成过程中。

这会显示如何实现`Extra.cs`文件中的代码。 请注意, 你将使用分部类, 因为这会增加从`ApiDefinition.cs` `StructsAndEnums.cs`和核心绑定的组合生成的分部类:
...
本机库按约定从单词 "lib" 开始, 以扩展名 ". a" 结尾。 当你执行此操作时, Visual Studio for Mac 将添加两个文件:。一个文件和一个C#自动填充的文件, 其中包含有关本机库包含的内容的信息:
 ...
 [![](objective-c-libraries-images/screen-shot-2012-02-08-at-3.45.06-pm.png "本机库按约定从 word lib 开始, 以扩展名结束。")](objective-c-libraries-images/screen-shot-2012-02-08-at-3.45.06-pm.png#lightbox)

此`libMagicChord.linkwith.cs`文件的内容包含有关如何使用此库的信息, 并指示 IDE 将此二进制文件打包到生成的 DLL 文件中:
...
最简单的绑定是绑定方法。 只需使用C#命名约定在接口中声明方法, 并使用[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute)
attribute. 属性用于将你C#的名称与 Xamarin 运行时中的目标-C 名称进行链接。 [`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute) 的参数。[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute) 
...
这是必需的, 因为协定是接口的一部分, 并且接口没有静态和实例声明的概念, 因此, 需要再次使用特性。 如果要从绑定中隐藏某个特定方法, 则可以使用[`[Internal]`](~/cross-platform/macios/binding/binding-types-reference.md#InternalAttribute)特性修饰该方法。
...
导出引用类型时, 还可以使用[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute)关键字指定分配语义。 这是确保不泄露数据所必需的。
...
属性并直接映射到C#属性。 与方法一样, 属性可以用[`[Static]`](~/cross-platform/macios/binding/binding-types-reference.md#StaticAttribute)
和[`[Internal]`](~/cross-platform/macios/binding/binding-types-reference.md#InternalAttribute)
属性.

当你在 btouch [`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute)中的属性上使用属性时, 实际上会绑定两种方法: getter 和 setter。 你为导出提供的名称是**basename** , 而 setter 是通过预先计算 "set" 一词来计算的, 将**basename**的第一个字母变为大写, 并使选择器采用参数。 这意味着`[Export ("label")]`应用于属性的实际上是绑定 "标签" 和 "setLabel:"目标-C 方法。
...
除了使用[`[Static]`](~/cross-platform/macios/binding/binding-types-reference.md#StaticAttribute)对静态属性的支持外, 还可以使用修饰线程静态[`[IsThreadStatic]`](~/cross-platform/macios/binding/binding-types-reference.md#IsThreadStaticAttribute)属性, 例如:
...
与方法允许标记[`[NullAllowed]`](~/cross-platform/macios/binding/binding-types-reference.md#NullAllowedAttribute)某些参数一样, 你可以应用[`[NullAllowed]`](~/cross-platform/macios/binding/binding-types-reference.md#NullAllowedAttribute)
...
还可以直接在 setter 上指定[参数:`[NullAllowed]`](~/cross-platform/macios/binding/binding-types-reference.md#NullAllowedAttribute)
...
这种映射到C#的方式是在基类上同时添加 getter 和 setter, 但使用[`[NotImplemented]`](~/cross-platform/macios/binding/binding-types-reference.md#NotImplementedAttribute)
attribute.
...
由于这是对定义`UIAccelerometerDelegate`的可选方法, 因此不需要执行任何其他操作。 但如果协议上有必需的方法, 则应将[`[Abstract]`](~/cross-platform/macios/binding/binding-types-reference.md#AbstractAttribute)
方法的特性。 这会强制该实现的用户实际提供该方法的主体。

通常, 协议用于响应消息的类。 此操作通常在目标-C 中完成, 方法是向 "委托" 属性分配一个响应协议中的方法的对象的实例。

Xamarin 中的约定是支持目标-C 松耦合样式, `NSObject`可在其中将的任何实例分配给委托, 还可以公开其强类型版本。 出于此原因, 我们通常同时`Delegate`提供强类型的属性`WeakDelegate`和具有松散类型的。 通常[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute), 我们会将松类型版本绑定到, 并[`[Wrap]`](~/cross-platform/macios/binding/binding-types-reference.md#WrapAttribute)使用属性提供强类型版本。
...
对于目标为 C `MyProtocol`的每个协议定义, 现在提供了`IMyProtocol`一个接口, 该接口列出了协议所需的所有方法以及提供所有可选方法的扩展类。  以上与 Xamarin Studio 编辑器中的新支持结合使用, 使开发人员可以实现协议方法, 而不必使用前面抽象模型类的单独子类。
...
例如, 在 Xamarin 中, 我们绑定了在`NSString`时`UIKit`定义的扩展方法`NSStringDrawingExtensions`, 如中所示:
...
如果要包装静态类中不是从派生的`NSObject`各个字段, 可以使用[`[Static]`](~/cross-platform/macios/binding/binding-types-reference.md#StaticAttribute_Class) 
类上的特性, 如下所示:
...
上面将生成一个, `LonelyClass`它不是从`NSObject`派生的, 并且将包含绑定`NSString`到`NSSomeEventNotification`作为的公开的
 `NSString` 。
...
你可以在`enum`绑定文件中直接添加, 以便更轻松地在 API 定义中使用它们, 而无需使用不同的源文件 (需要在绑定和最终项目中进行编译)。
...
在上面的示例中, 您可以决定`void Perform (NSString mode);` [`[Internal]`](~/cross-platform/macios/binding/binding-types-reference.md#InternalAttribute)使用特性进行修饰。 这将**隐藏**绑定使用者的基于常量的 API。

但这会限制类型为子类, 因为更好 API 替代项使用[`[Wrap]`](~/cross-platform/macios/binding/binding-types-reference.md#WrapAttribute)特性。 这些生成的方法不`virtual`是, 也就是说, 您不能覆盖它们, 这可能是一个不错的选择。

一种替代方法是将基于的`NSString`原始、定义标记为`[Protected]`。 这将允许在需要时进行子类工作, 并且 wrap'ed 版本仍将正常工作并调用重写的方法。

### <a name="binding-nsvalue-nsnumber-and-nsstring-to-a-better-type"></a>将`NSValue`、 `NSNumber`和`NSString`绑定到更好的类型

[`[BindAs]`](~/cross-platform/macios/binding/binding-types-reference.md#BindAsAttribute)特性允许`NSValue`将和`NSString`(枚举) 转换为更准确C#的类型。 `NSNumber` 特性可用于通过本机 API 创建更好、更准确的 .NET API。

可以用[`[BindAs]`](~/cross-platform/macios/binding/binding-types-reference.md#BindAsAttribute)来修饰方法 (返回值)、参数和属性。 唯一的限制是, 成员**不**能在[`[Protocol]`](~/cross-platform/macios/binding/binding-types-reference.md#ProtocolAttribute) 
...
`CAScroll`是一个`NSString`受支持的枚举, 我们将提取`NSString`适当的值并处理类型转换。
...
上面将`MyScreenChangedEventArgs`生成一个`ScreenX`包含和`ScreenY`属性的类, 这些属性将分别使用关键字 "ScreenXKey" 和 "ScreenYKey" 提取[NSNotification](xref:Foundation.NSNotification.UserInfo)字典中的数据, 并正确应用转化. 如果在中设置了该键`UserInfo`, 则该特性用于探测,而不是`[ProbePresence]`尝试提取值。 这适用于出现密钥的情况是值 (通常用于布尔值)。
...
类别是一个目标 C 机制, 用于扩展类中可用的一组方法和属性。   在实践中, 当在中`NSObject` `UIKit`链接特定框架时 (例如), 它们用于扩展基类的功能, 从而使其方法可用, 但只有在新框架已链接的情况下才可用。   在某些其他情况下, 它们用于按功能组织类中的功能。   它们在本质上与C#扩展方法相似。这是一种类别在目标-C 中的外观:
...
如果在库中找到, 则使用方法`UIView` `makeBackgroundRed`扩展的实例。

若要绑定这些[`[Category]`](~/cross-platform/macios/binding/binding-types-reference.md#CategoryAttribute)属性, 可以在接口定义上使用特性。  当使用[`[Category]`](~/cross-platform/macios/binding/binding-types-reference.md#CategoryAttribute)
...
上面的将创建`MyUIViewExtension`一个`MakeBackgroundRed`包含扩展方法的类。  这意味着你现在可以在任何`UIView`子类上调用 "MakeBackgroundRed", 为你提供的功能与在目标 C 上所获得的功能相同。 在某些其他情况下, 类别用于扩展系统类, 但只是为了进行装饰而对功能进行组织。  如：
...
上述说明声明一个名为`enumerateObjectsUsingBlock:`的方法, 该方法采用一个名为`block`的参数。 此块类似于C#匿名方法, 因为它支持捕获当前环境 ("this" 指针、对局部变量和参数的访问)。 中`NSSet`的上述方法使用两个`NSObject`参数 ( `id obj`部分) `BOOL *stop`和一个指向布尔值的指针 () 调用块。
...
返回 void 的方法的属性, 其最后一个参数是回调。  将此应用于方法时, 绑定生成器将使用后缀`Async`生成该方法的版本。  如果回调不使用参数, 则返回值将为`Task`, 如果回调采用参数, 则结果将`Task<T>`为。  如果回调采用多个参数, 则应设置`ResultType`或`ResultTypeName`以指定所需的生成类型名称, 该名称将包含所有属性。
...
在目标-C API 的很多位置, 参数将作为具有特定键和`NSDictionary`值的弱类型 api 进行传递, 但这种情况容易出错 (你可以传递无效的键, 无警告; 你可以传递无效的值, 并且无警告), 并使若要使用, 需要多次尝试文档才能查找可能的键名和值。

解决方案是提供强类型的版本, 该版本提供 API 的强类型版本, 并在后台映射各种基础键和值。

例如, 如果目标为 C 的 API `NSDictionary`已接受, 并且它被记录为采用从0.0 到1.0 的`XyzCaptionKey`卷`NSNumber`值和采用字符串的密钥`XyzVolumeKey` , 则需要用户拥有一个优秀的 API如下所示:
...
用于显示新 API 入口点的属性, 并使用强类型化`XyzOptions`类对其进行介绍。  包装方法还允许传递 null。

现在, 我们没有提到的一件事就是这些`XyzOptionsKeys`值的来源。  通常会按如下`XyzOptionsKeys`所示对 API 在静态类中的表面进行分组:
...
若要创建强类型的字典, 请在 API 中引入一个接口, 并使用[StrongDictionary](~/cross-platform/macios/binding/binding-types-reference.md#StrongDictionary)属性对其进行修饰。  这会告知生成器应创建一个与将从`DictionaryContainer`派生的接口同名的类, 并为其提供强类型访问器。
...
如果需要在`XyzOption`成员中引用不同的字段 (这不是带有后缀`Key`的属性的名称), 则可以使用[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute) 
要使用的名称的特性。
...
Xamarin 运行时自动处理将数组C#转换为`NSArrays`并执行转换的过程, 例如, 返回`NSArray`的的`UIViews`虚目标 C 方法:
...
选择器在目标 C API 上显示为特殊类型`SEL`。 绑定选择器时, 需要将类型映射到`ObjCRuntime.Selector`。  通常, 选择器会在具有对象、目标对象和选择器的 API 中公开, 以便在目标对象中调用。 同时提供这两种方法基本上对应C#于委托: 封装要调用的方法的对象以及用于调用方法的对象。
...
为了C#使绑定更好开发人员, 通常会提供一个方法, 该方法采用`NSAction`参数, 这允许C#使用委托和 lambda 而不是`Target+Selector`。 为此, 通常`SetTarget`可以通过使用[`[Internal]`](~/cross-platform/macios/binding/binding-types-reference.md#InternalAttribute)
...
当你可能想要`NSString`直接使用时, 唯一的情况是将该字符串用作标记。 有关字符串和`NSString`的详细信息, 请参阅[NSString 文档上的 API 设计](~/ios/internals/api-design/nsstring.md)。
...
用于指定要生成的`EventArgs`类的名称。 `EventArgs` 应为`EventArgs` `With`每个签名使用一个 (在此示例中, 将包含 nint 类型的属性)。
...
回调与事件调用相同, 不同之处在于, 不是具有多个潜在的订阅服务器 (例如, 多个方法可以`Clicked`挂钩到事件`DownloadFinished`或事件) 回调只能有一个订阅服务器。

此过程是相同的, 唯一的区别在于, 通常使用 EventArgs 来命名生成`EventArgs`的C#委托名称, 而不是公开将生成的类的名称。
...
你需要通知 Xamarin iOS 如何链接库, 这可以通过以下方式完成: 更改生成配置以使用一些额外的生成参数`mtouch`来调用命令, 这些自变量指定如何使用 "-gcc_flags" 选项链接到新库。后跟一个带引号的字符串, 其中包含程序所需的所有额外库, 如下所示:
...
您可能想知道, 为什么您需要`-force_load`命令, 原因是-ObjC 标志尽管它在中编译代码, 但它不会保留支持类别所需的元数据 (链接器/编译器死代码去除块)。Xamarin 时需要运行时。
...
例如, 如果你有一个类, 其中显示一条消息, 然后`Done`生成事件, 则处理此事件的传统方法是:
...
若要利用这一点, 你的类应在[`[BaseType]`](~/cross-platform/macios/binding/binding-types-reference.md#BaseTypeAttribute)声明中设置 Events 属性, 并`KeepUntilRef`将变量设置为在对象完成其工作时调用的方法的名称, 如下所示:
...
自 Xamarin 版本3.2 开始, 我们支持从已用[`[Model]`](~/cross-platform/macios/binding/binding-types-reference.md#ModelAttribute)属性标记的协议继承。 这在某些 API 模式下非常有用, 例如在`MapKit`中`MKOverlay` , `MKAnnotation`协议从协议继承, 并被多个继承自`NSObject`的类采用。

在过去, 我们需要将协议复制到每个实现中, 但在这种情况`MKShape`下, 我们可以`MKOverlay`让类从协议继承, 并且它将自动生成所有所需的方法。

...
构建跨平台应用程序的一个关键组件是能够跨不同于平台的项目共享代码。 但是，这种情况很复杂，这是因为不同的平台通常使用不同的 .NET 基类库（BCL）的子集，因此实际构建到不同的 .NET Core 库配置文件中。 这意味着，每个平台只能使用以相同配置文件为目标的类库，因此它们似乎需要为每个平台使用单独的类库项目。
...
可以在[Microsoft 网站](https://msdn.microsoft.com/library/gg597391(v=vs.110).aspx)上阅读有关不同配置文件功能的详细信息，并查看另一个社区成员的[PCL 配置文件摘要](https://portablelibraryprofiles.stephencleary.com/)，其中包括受支持的框架信息和其他说明。
...
1. 因为在多个应用程序之间共享相同的可移植类库，所以不能引用平台特定的库（例如 CsharpSqlite. WP7）。
...
[用于设置配置文件![PCL 项目选项](pcl-images/image4-sml.png)](pcl-images/image4.png#lightbox)
...
[PCL 的 System.IO 类中14类![Intellisense 列表](pcl-images/image6.png)](pcl-images/image6.png#lightbox)

将其与 Xamarin 或 Xamarin 项目中的 System.IO 自动完成进行比较-有40类可用，包括 `File` 和 `Directory` 等常用类，这些类不在任何 PCL 配置文件中。

[.NET Framework System.IO 命名空间中的40类![Intellisense 列表](pcl-images/image7.png)](pcl-images/image7.png#lightbox)
...
[显示 PCL 项目![TaskyPortable 示例解决方案](pcl-images/image9.png)](pcl-images/image9.png#lightbox)
...
3. PCL 项目将显示在解决方案资源管理器中，&ndash; 文本 **（可移植）** 出现在项目名称旁边以指示它是 PCL：
...
将其与常规项目中的 System.IO 自动完成进行比较–存在40个类，其中包括 `File` 和 `Directory` 等常用类，这些类不在任何 PCL 配置文件中。

[![多个 .NET Framework 中可用的 IO 类](pcl-images/image15.png)](pcl-images/image15.png#lightbox)
...
创建 PCL 项目后，可以采用与通常添加引用相同的方式从任何兼容的应用程序或库项目中添加对该项目的引用。 在 Visual Studio 中，右键单击 "引用" 节点，然后选择 "`Add Reference...` 然后切换到**解决方案" > 项目**"选项卡，如下所示：
...
[显示 PCL 库![TaskyPortable 示例解决方案](pcl-images/image17.png)](pcl-images/image17.png#lightbox)
...
可移植类库受支持的 .NET 功能的限制。 由于它编译为在多个平台上运行，因此它不能使用在 SQLite-NET 中使用的 `[DllImport]` 功能。 相反，SQLite 作为抽象类实现，然后通过其余共享代码进行引用。 抽象 API 的提取如下所示：
...
这些项目还包含在该平台上运行的抽象数据库 API 的实现。 在 iOS 和 Android 上，Sqlite 数据库引擎内置于操作系统中，因此实现可以使用 `[DllImport]`，如提供数据库连接的具体实现。 下面显示了特定于平台的实现代码摘录：

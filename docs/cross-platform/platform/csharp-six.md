...
上一示例中的最后一行显示了`?` `??`与 null 合并运算符组合在一起的 null 条件运算符。 新C#的 6 null 条件运算符在数组`null`中的第2个元素上返回，此时 null 合并运算符将在中启动，并向`lengths`数组提供0（当然，无论是否适用）。特定于问题）。

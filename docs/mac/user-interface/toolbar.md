...
另请查看[Xamarin 内部](~/mac/internals/how-it-works.md)版文档的[公开C#类/方法到目标-C](~/mac/internals/how-it-works.md)部分。 它说明了用于将类连接C#到目标 C 类的 `Register` 和 `Export` 属性。
...
1. 在 Visual Studio for Mac 的 " **Solution Pad**中，右键单击" **Resources** "文件夹，然后选择"**添加** > **添加文件**"。
...
若要禁用工具栏上的项，请创建一个自定义 `NSToolbarItem` 类并重写 `Validate` 方法。 然后，在 Interface Builder 中，将自定义类型分配给要启用/禁用的项。
...
为**垃圾桶**工具栏项创建一个名为 `trashItem` 的插座。 保存更改并返回到 Visual Studio for Mac 以与 Xcode 同步。 最后，打开**MainWindow.cs**并将 `AwakeFromNib` 方法更新为，如下所示：

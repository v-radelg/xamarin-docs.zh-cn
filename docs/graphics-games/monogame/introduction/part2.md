...
不熟悉游戏编程的开发人员可能会想知道`float`为什么要`int`使用该类型，而不`double`是或。 原因在于，32位值最常用于在低级别呈现代码中定位。 Double 类型占用64位的精度，这对于定位对象很少需要。 尽管32位差异看起来不是重要的，但许多新式游戏都包含需要处理每个帧每个帧（每秒30或60次）的数十个位置值的图形。 将必须通过图形管道移动的内存量减少一半可能会对游戏性能产生显著影响。

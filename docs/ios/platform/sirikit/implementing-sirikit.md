...
1. 双击`Entitlements.plist`文件中**解决方案资源管理器**打开进行编辑。
2. 添加`com.apple.developer.siri`**属性**，将**类型**到`Boolean`并**值**到`Yes`: 
...
3. 保存对文件所做的更改。
4. 双击**项目文件**中**解决方案资源管理器**打开进行编辑。
5. 选择**iOS 捆绑签名**并确保`Entitlements.plist`中选择文件**自定义权利**字段。
...
19. 絋粄**捆绑包标识符**匹配更高版本的 Apple 的开发人员门户中创建的一个： 

    [![](implementing-sirikit-images/setup06.png "捆绑包标识符")](implementing-sirikit-images/setup06.png#lightbox)
20. 在中**解决方案资源管理器**，选择**项目**。
21. 右键单击项目并选择**选项**。
22. 选择**iOS 捆绑签名**，选择**签名标识**并**预配配置文件**上面创建： 
...
4. 在中**解决方案资源管理器**，右键单击**引用**新建的 Intents 扩展的文件夹，然后选择**添加 > 引用**。 检查常见的共享的代码库项目 （即上面创建的应用） 的名称，然后单击**确定**按钮：
...
2. 从对话框中选择**iOS** > **扩展** > **意向 UI 扩展**然后单击**下一步**按钮。
3. 接下来输入**名称**意向扩展并单击**确定**按钮。
4. 在中**解决方案资源管理器**，右键单击**引用**新创建的意向扩展的文件夹。 检查常见的共享的代码库项目 （即上面创建的应用） 的名称，然后单击**确定**按钮。

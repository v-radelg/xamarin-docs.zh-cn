...
新重写`UpdatedDownloads`的方法如下所示。 应用商店工具包在中`StartDownloads` `UpdatedTransactions`触发后调用此方法。 此方法将按不确定的*间隔多次调用*，以提供下载进度，并在下载完成后再次进行。 请注意，方法接受对象的`SKDownload`数组，因此每个方法调用都可以在队列中提供多个下载的状态。 如以下实现中所示，每次都会检查下载状态，并采取适当的措施。

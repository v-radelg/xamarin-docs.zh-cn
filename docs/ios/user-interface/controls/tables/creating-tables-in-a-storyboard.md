...
具有原型内容的 `UITableView` 通常用于显示一个数据列表，其中的原型单元（或可定义多个单元格）对列表中的每个项重复使用。 单元不需要实例化，而是通过调用其 `UITableViewSource`的 `DequeueReusableCell` 方法在 `GetView` 方法中获取。

 <a name="Static_Content" />

## <a name="static-content"></a>静态内容

利用静态内容的 `UITableView`，可以在设计图面上直接设计表。 可以通过更改属性并添加控件，将单元拖到表中并进行自定义。
...
该解决方案将打开，其中C#包含一些文件和一个已创建的`Main.storyboard`文件。 双击 `Main.storyboard` 文件以在 iOS 设计器中将其打开。
...
首次更改情节提要时，会删除现有的详细信息视图，并将其替换为 UITableViewController。 请执行这些步骤：
...
4. 选择创建的新 segue，并为其指定一个标识符以在代码中引用此 segue。 单击 "segue" 并在**Properties Pad**中输入**标识符**`TaskSegue`，如下所示：    
  [属性面板中的![命名 segue](creating-tables-in-a-storyboard-images/image16a-sml.png)](creating-tables-in-a-storyboard-images/image16a.png#lightbox) 

5. 接下来，通过选择两个表视图并使用 Properties Pad 进行配置。 请确保选择 "查看"，而不是 "查看控制器" –您可以使用文档大纲来帮助选择。

6. 将根视图控制器更改为**Content：动态原型**（Design Surface 上的视图将作为**原型内容**的标签）：
...
8. 新的 UITableViewController 必须具有其类名称和标识符集。 在**Properties Pad**中为**类**选择视图控制器并键入_TaskDetailViewController_ –这将在 Solution Pad 中创建新的 `TaskDetailViewController.cs` 文件。 输入**StoryboardID**作为_详细信息_，如以下示例中所示。 稍后将用于在代码中C#加载此视图：  
...
[导航栏中的![条形按钮项](creating-tables-in-a-storyboard-images/image23-sml.png)](creating-tables-in-a-storyboard-images/image23.png#lightbox)

请执行以下操作： 

- 将 "**条形" 按钮项**从工具箱拖动到_导航栏的右侧_。
- 在**Properties Pad**的 "**条形按钮项**" 下，选择 "**标识符：添加**" （使其成为 *+* 加按钮）。 
...
- 将两个按钮拖到单元格上，并适当地设置其标题（如_保存_并_删除_），如下所示：
   ...
   [![在底部设置两个按钮](creating-tables-in-a-storyboard-images/image30-sml.png)](creating-tables-in-a-storyboard-images/image30.png#lightbox)
...
首先，我们要创建一个 `Chores` 类，这将提供一种方法来获取和设置 ID、名称、注释和已完成的布尔值，以便我们可以在整个应用程序中使用这些值。
...
接下来，创建一个继承自 `UITableViewSource`的 `RootTableSource` 类。 

此表和非情节提要表视图的区别在于 `GetView` 方法不需要实例化任何单元– `theDequeueReusableCell` 方法将始终返回原型单元的实例（具有匹配标识符）。

下面的代码来自 `RootTableSource.cs` 文件：
...
若要使用 `RootTableSource` 类，请在 `ItemViewController`的构造函数中创建新的集合：
...
在 `ViewWillAppear` 将集合传递到源并分配给表视图：
...
若要在 segue 中 "发送参数"，请重写 `PrepareForSegue` 方法，并设置 `DestinationViewController` 上的属性（在本示例中为 `TaskDetailViewController`）。 目标视图控制器类将已经实例化但尚未向用户显示-这意味着你可以设置类的属性，但不能修改任何 UI 控件：
...
在 `TaskDetailViewController` `SetTask` 方法会将其参数分配给属性，以便在 ViewWillAppear 中引用它们。 无法在 `SetTask` 中修改控件属性，因为调用 `PrepareForSegue` 时可能不存在：
...
接下来，需要将按钮的 `TouchUpInside` 事件处理程序添加到**TaskDetailViewController.cs**的 `ViewDidLoad` 方法。 对 `ItemViewController` 的 `Delegate` 属性引用是专门创建的，因此我们可以调用 `SaveTask` 和 `DeleteTask`，这会在其操作过程中关闭此视图：
...
要生成的最后一项功能是创建新任务。 在**ItemViewController.cs**中，添加一个方法，用于创建新任务并打开详细信息视图。 若要从情节提要实例化视图，请将 `InstantiateViewController` 方法用于该视图的 `Identifier`-在此示例中，将为 "detail"：
...
最后，将导航栏中的按钮连接到**ItemViewController.cs**的 `ViewDidLoad` 方法来调用它：

...
由于 iPhone 上使用 Xamarin.iOS 开发的应用程序会被编译为静态代码，因此任何需要在运行时生成代码的机制都无法使用。
...
### <a name="pinvokes-in-generic-types"></a>泛型类型的 P/Invoke	 
...
当前不支持使用反射的 Property.SetInfo 来设置可以为 Null 的&lt;T&gt;上的值。
...
使用值类型作为 Dictionary&lt;TKey，TValue&gt; 键存在问题，因为默认的 Dictionary 构造函数尝试使用 EqualityComparer&lt;TKey&gt;.Default。反过来，EqualityComparer&lt;TKey&gt;.Default 则尝试使用反射来实例化可实现 IEqualityComparer&lt;TKey&gt; 接口的新类型。

这适用于引用类型（因为已跳过“反射+创建新类型”这一步骤），但对于值类型，只要尝试在设备上使用，很快便会崩溃。
...
由于 iOS 内核会阻止应用程序动态生成代码，因此 Xamarin.iOS 不支持任何形式的动态代码生成。其中包括：
 ...
 **重要提示：** 请不要将 Reflection.Emit 与 Reflection 混淆。Reflection.Emit 涉及动态生成代码并将代码即时编译为本机代码。由于 iPhone 上的限制（没有 JIT 编译），因此不支持此功能********。

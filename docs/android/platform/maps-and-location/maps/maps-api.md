...
[![已选择 GooglePlayServices 包的 NuGet 包管理器](maps-api-images/image03-sml.png)](maps-api-images/image03.png#lightbox)
...
- **访问网络状态**&ndash; Map API 必须能够检查是否可下载地图图块。

- **Internet 访问**&ndash;若要下载地图磁贴并与用于 API 访问的 Google Play 服务器进行通信，需要访问 Internet。
...
- **请求旧版 APACHE HTTP 客户端**&ndash;面向 Android 9.0 （API 级别28）或更高版本的应用程序必须指定旧的 Apache HTTP 客户端是要使用的可选库。

- **访问 Google 基于 Web 的服务**&ndash;应用程序需要权限才能访问 Google 的 web 服务，该服务返回 Android Maps API。

- **Google Play Services 通知的权限**&ndash;必须授予应用程序从 Google Play Services 接收远程通知的权限。
...
`GoogleMap`通过以下两种方式之一添加到活动中：

- **MapFragment** - [MapFragment](https://developers.google.com/android/reference/com/google/android/gms/maps/MapFragment)是作为`GoogleMap`对象主机的专用片段。 `MapFragment`需要 Android API 级别12或更高版本。
...
- **MapView** - [MapView](https://developers.google.com/android/reference/com/google/android/gms/maps/MapView)是专用的视图子类，可充当`GoogleMap`对象的主机。 此类的用户必须将所有活动生命周期方法转发给`MapView`该类。

其中每个容器都公开`Map`一个属性，该属性返回`GoogleMap`的实例。 应为[MapFragment](https://developers.google.com/android/reference/com/google/android/gms/maps/MapFragment)类提供首选项，因为它是一个更简单的 API，可减少开发人员必须手动实现的样本代码量。

### <a name="adding-a-mapfragment-to-an-activity"></a>向活动添加 MapFragment

下面的屏幕截图是一个简单`MapFragment`的示例：

[![显示 Google 地图片段的设备屏幕截图](maps-api-images/image05-sml.png)](maps-api-images/image05.png#lightbox)

与其他片段类类似，可以通过两种方式将添加`MapFragment`到活动中：

- **以声明方式**- `MapFragment`可以通过活动的 XML 布局文件添加。 以下 XML 代码片段演示了如何使用`fragment`元素的示例：
...
- **以编程方式**- `MapFragment`可以使用[`MapFragment.NewInstance`](https://developers.google.com/android/reference/com/google/android/gms/maps/MapFragment.html#newInstance())方法以编程方式实例化，然后添加到活动。 此代码片段演示实例化`MapFragment`对象并添加到活动的最简单方法：
    ...
    可以通过将[`GoogleMapOptions`](https://developers.google.com/android/reference/com/google/android/gms/maps/GoogleMapOptions)对象传递到`MapFragment` `NewInstance`来配置对象。 本指南后面的 " [GoogleMap 属性](#googlemap_object)" 部分对此进行了讨论。

方法用于初始化由片段承载`MapFragment`的[，并获取对承载的地图对象的引用。`GoogleMap`](#googlemap_object) `MapFragment.GetMapAsync` 此方法使用一个实现`IOnMapReadyCallback`接口的对象。

此接口具有一个方法，当`IMapReadyCallback.OnMapReady(MapFragment map)`应用可能与`GoogleMap`对象交互时，将调用此方法。 下面的代码片段演示了 Android 活动如何初始化`MapFragment`并`IOnMapReadyCallback`实现接口：
...
`GoogleMap`定义多个属性，这些属性可控制地图的功能和外观。 配置的初始状态`GoogleMap`的一种方法是，在创建时传递[GoogleMapOptions](https://developers.google.com/android/reference/com/google/android/gms/maps/GoogleMapOptions) `MapFragment`对象。 以下代码片段是在`GoogleMapOptions` `MapFragment`创建时使用对象的一个示例：
...
配置的另一`GoogleMap`种方法是操作地图对象的[UiSettings](https://developers.google.com/android/reference/com/google/android/gms/maps/UiSettings)的属性。 下面的代码示例演示如何配置`GoogleMap`以显示缩放控件和罗盘：
...
基于 Mercator 投影，地图在屏幕上建模为平面平面。 "地图" 视图是指在此平面上向下看的*相机*的图面。 可以通过更改位置、缩放、倾斜和轴承来控制照相机的位置。 [CameraUpdate](https://developers.google.com/android/reference/com/google/android/gms/maps/CameraUpdate)类用于移动照相机位置。 `CameraUpdate`对象不是直接实例化的，而映射 API 则提供[CameraUpdateFactory](https://developers.google.com/android/reference/com/google/android/gms/maps/CameraUpdateFactory)类。

创建对象后，该对象将作为参数传递给[GoogleMap 或 MoveCamera](https://developers.google.com/android/reference/com/google/android/gms/maps/GoogleMap#moveCamera(com.google.android.gms.maps.CameraUpdate)) [方法。](https://developers.google.com/android/reference/com/google/android/gms/maps/GoogleMap#animateCamera(com.google.android.gms.maps.CameraUpdate)) `CameraUpdate` 方法可即时更新映射， `AnimateCamera`同时方法提供平滑的动画转换。 `MoveCamera`

此代码段是一个简单的示例，演示如何使用`CameraUpdateFactory`创建一个`CameraUpdate` ，它会将地图的缩放级别增加一个缩放级别：
...
Maps API 提供了一个[CameraPosition](https://developer.android.com/reference/com/google/android/gms/maps/model/CameraPosition.html) ，它将为相机位置聚合所有可能的值。 此类的实例可以提供给将返回`CameraUpdate`对象的[CameraUpdateFactory. NewCameraPosition](https://developers.google.com/maps/documentation/android/reference/com/google/android/gms/maps/CameraUpdateFactory#newCameraPosition%28com.google.android.gms.maps.model.CameraPosition%29)方法。 Maps API 还包括[CameraPosition](https://developer.android.com/reference/com/google/android/gms/maps/model/CameraPosition.Builder.html)类，该类提供用于创建`CameraPosition`对象的 Fluent API。
下面的代码段演示了如何`CameraUpdate` `CameraPosition`从创建，并使用来更改上`GoogleMap`的照相机位置的示例：
...
在前面的代码片段中，地图上的特定位置由[LatLng](https://developers.google.com/android/reference/com/google/android/gms/maps/model/LatLng)类表示。 "缩放级别" 设置为18，这是 Google Maps 使用的缩放的任意度量。 轴承是从北顺时针测量的方向。 倾斜属性控制查看角度，并指定从垂直方向25度。 下面的屏幕截图显示`GoogleMap`了执行上述代码后的：

[![示例 Google Map 显示具有倾斜角度的指定位置](maps-api-images/image06-sml.png)](maps-api-images/image06.png#lightbox)
...
[![用于 Vimy 凸的标记和信息窗口示例 Google Map](maps-api-images/image07-sml.png)](maps-api-images/image07.png#lightbox)

##### <a name="customizing-a-marker"></a>自定义标记

将标记添加到地图时，可以通过调用`MarkerOptions.InvokeIcon`方法来自定义标记使用的图标。
此方法采用包含呈现图标所需的数据的[BitmapDescriptor](https://developers.google.com/android/reference/com/google/android/gms/maps/model/BitmapDescriptor)对象。 [BitmapDescriptorFactory](https://developers.google.com/android/reference/com/google/android/gms/maps/model/BitmapDescriptorFactory)类提供了一些帮助器方法来简化创建的过程`BitmapDescriptor`。 下面的列表介绍了其中一些方法：
...
- `FromFile(string fileName)`&ndash;根据指定路径处的文件创建自定义图标。

- `FromResource(int resourceId)`&ndash;根据指定的资源创建自定义图标。
...
- `public View GetInfoWindow(Marker marker)`&ndash;调用此方法可获取标记的自定义信息窗口。 如果返回`null` ，则将使用默认的窗口呈现。 如果此方法返回一个视图，则该视图将放在信息窗口框架中。

- `public View GetInfoContents(Marker marker)`仅当 GetInfoWindow 返回`null`时，才会调用此方法。 &ndash; 如果要使用信息窗口`null`内容的默认呈现，则此方法可以返回一个值。 否则，此方法应返回具有信息窗口内容的视图。
...
[![带极坐标的重叠图像的示例映射](maps-api-images/image09-sml.png)](maps-api-images/image09.png#lightbox)
...
[折线](https://developers.google.com/android/reference/com/google/android/gms/maps/model/Polyline)是连续`LatLng`对象的列表，这些对象指定每个线段的顶点。 折线是通过先创建`PolylineOptions`对象并向其中添加点来创建的。 然后，通过调用`GoogleMap` `AddPolyline`方法将对象传递给对象。`PolylineOption`
...
`Polygon`s 类似于`Polyline`s，但它们未打开。 `Polygon`s 为关闭循环，并填充其内部。
`Polygon`的创建方式`Polyline`与相同，不同之处在于调用[GoogleMap. AddPolygon](https://developers.google.com/android/reference/com/google/android/gms/maps/GoogleMap.html#addPolygon(com.google.android.gms.maps.model.PolygonOptions))方法。

与不同`Polyline`的`Polygon`是，是自结束。 通过绘制一条线来连接第`AddPolygon`一个点和最后一个点，该方法将关闭多边形。 下面的代码段将在与`Polyline`示例中的上一个代码段相同的区域上创建一个实心矩形。
...
当`MarkerClicked`用户点击标记时，将引发事件。 此事件接受`GoogleMap.MarkerClickEventArgs`对象作为参数。 此类包含两个属性：

- `GoogleMap.MarkerClickEventArgs.Handled`应将此属性设置为`true` ，以指示事件处理程序已使用该事件。 &ndash; 如果此设置为， `false`则除了事件处理程序的自定义行为外，还会发生默认行为。

- `Marker`此属性是对`MarkerClick`引发事件的标记的引用。 &ndash;

此代码片段演示了一个`MarkerClick`示例，该示例将相机位置更改为地图上的新位置：
...
当用户希望拖动标记时，将引发此事件。 默认情况下，不能拖动标记。 可以通过将`Marker.Draggable`属性设置为`true`或以参数`true`形式调用`MarkerOptions.Draggable`方法，将标记设置为可拖动标记。

若要拖动标记，用户必须首先长时间单击该标记，然后其手指必须保留在地图上。 当用户的手指在屏幕上拖动时，标记将移动。 当用户的手指从屏幕上移开时，标记会保持不变。

以下列表描述了可拖动标记将引发的各种事件：

- `GoogleMap.MarkerDragStart(object sender, GoogleMap.MarkerDragStartEventArgs e)`&ndash;当用户第一次拖动标记时，将引发此事件。

- `GoogleMap.MarkerDrag(object sender, GoogleMap.MarkerDragEventArgs e)`&ndash;拖动标记时引发此事件。

- `GoogleMap.MarkerDragEnd(object sender, GoogleMap.MarkerDragEndEventArgs e)`&ndash;当用户完成拖动标记后，将引发此事件。

每个都`P0` `Marker`包含一个名为的属性，该属性是对要拖动的对象的引用。 `EventArgs`

### <a name="info-window-click-events"></a>信息窗口单击事件

一次只能显示一个 "信息" 窗口。 当用户单击地图中的信息窗口时，地图对象将引发一个`InfoWindowClick`事件。 下面的代码段演示如何将处理程序绑定到事件：
...
回忆一下，"信息" 窗口是`View`一个静态的，它呈现为地图上的图像。 放置在 "信息" 窗口中的任何小组件（例如按钮、复选框或文本视图）都将静态，无法响应其任何重要用户事件。

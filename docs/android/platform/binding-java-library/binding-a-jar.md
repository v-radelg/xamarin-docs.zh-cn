...
在本指南中, 我们将逐步介绍为单一的绑定库创建基础知识。JAR 文件。 我们将通过一个示例&ndash;来说明, 其中一切都是必需的, 无需对绑定进行自定义或调试。 
...
在下面的演练中, 我们将创建[毕加索](http://square.github.io/picasso/)的绑定库, 这是一个流行的 Android。提供图像加载和缓存功能的 JAR。 我们将使用以下步骤绑定**picasso-2**以创建可在 Xamarin Android 项目中使用的新 .net 程序集: 
...
[![选择 jar 文件, 并单击 "添加"](binding-a-jar-images/03-select-jar-file-sml.png)](binding-a-jar-images/03-select-jar-file.png#lightbox)
...
通常, 使用**EmbeddedJar**生成操作, 以便。JAR 自动打包到绑定库中。 这是中最简单&ndash;的 Java 字节码。JAR 转换为 Dex 字节码, 并嵌入到 APK 中 (以及托管的可调用包装器)。 如果要保留, 则为。JAR 与绑定库分离, 你可以使用**InputJar**选项;但是, 必须确保。JAR 文件在运行应用的设备上可用。 
...
设置绑定库的目标 API 级别 (在本示例中, 我们使用的是 API 级别 19): 
...
最后, 生成绑定库。 尽管可能会显示某些警告消息, 但绑定库项目应成功生成并生成输出。DLL 位于以下位置:**JarBinding/bin/Debug/JarBinding.dll**
...
在下面的步骤中, 我们将创建一个最小应用程序, 该应用程序使用绑定库下载并显示`ImageView`中的图像; "重提升" 由驻留在中的代码完成。JAR 文件。 
...
修改**BindingTest**布局 (**main.axml**), 使其`ImageView`具有一个:
...
将以下`using`语句添加到**MainActivity.cs** &ndash; , 这使得可以轻松访问位于绑定库中的基于`Picasso` Java 的类的方法:

```csharp
using Com.Squareup.Picasso;
```

修改方法, 使其`Picasso`使用类从 URL 加载图像并将`ImageView`其显示在中: `OnCreate` 
...
[![运行 BindingTest 的屏幕截图](binding-a-jar-images/11-result-sml.png)](binding-a-jar-images/11-result.png#lightbox)

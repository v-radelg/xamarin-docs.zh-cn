...
面向 Android 8.0 (API 级别 26) 或更高版本的应用程序可能无法静态注册隐式广播。 应用可能仍静态注册以进行显式广播。 此限制中有一小部分隐式广播列表。 Android 文档的[隐式广播例外](https://developer.android.com/guide/components/broadcast-exceptions.html)指南中介绍了这些例外。 对隐式广播感兴趣的应用程序必须使用`RegisterReceiver`方法动态执行此操作。 下面对此进行了介绍。

...
如果服务由于资源限制而终止，则服务将返回值 `StartCommandResult` 作为对 Android 的建议。 @No__t_0 有三个可能的值：
...
@No__t_0 &ndash; `StartCommandResult.ContinuationMask` 具有第四个值。 此值由 `OnStartCommand` 返回，它描述 Android 将如何继续已终止的服务。 此值通常不用于启动服务。

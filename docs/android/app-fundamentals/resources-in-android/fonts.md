...
1. **将字体打包为 Android 资源**&ndash;这可确保字体对应用程序始终可用, 但会增加 APK 的大小。
2. **下载字体**Android 还支持从_字体提供程序_下载字体。 &ndash; 字体提供程序检查字体是否已在设备上。 如有必要, 将在设备上下载并缓存字体。 可以在多个应用程序之间共享此字体。
...
只要以正确方式向 Android 应用程序提供字体, 就可以通过设置[ `fontFamily`属性](https://developer.android.com/reference/android/widget/TextView.html#attr_android:fontFamily)将它们应用于 UI 小组件。 例如, 以下代码片段演示了如何在 TextView 中显示字体:
...
定义字体系列后, 可以通过在布局文件中设置`fontFamily`、 `textStyle`和`fontWeight`属性以声明方式使用它。  例如, 下面的 XML 代码片段设置600权重字体 (普通) 和斜体文本样式:
...
可以通过使用[`Resources.GetFont`](https://developer.android.com/reference/android/content/res/Resources.html#getFont(int))方法[`Typeface`](https://developer.android.com/reference/android/graphics/Typeface.html)检索对象以编程方式设置字体。 许多视图都有`TypeFace`一个属性, 可用于将字体分配给小组件。 此代码段演示如何以编程方式在 TextView 上设置字体:
...
使用_字体提供程序_的帮助下载字体。 这是一种专用的内容提供程序, 用于管理将字体下载并缓存到设备上的所有应用程序的功能。 Android 8.0 包含用于从[Google 字体存储库](http://fonts.google.com)下载字体的字体提供程序。 此默认字体提供程序与 Android 支持库 v26 的 API 级别14向后移植。

当应用程序请求字体时, 字体提供程序将首先检查该字体是否已在设备上。 如果不是, 则它将尝试下载字体。 如果无法下载字体, 则 Android 将使用默认系统字体。 下载字体后, 设备上的所有应用程序都可以使用它, 而不只是发出初始请求的应用程序。

当请求下载字体时, 应用不会直接查询字体提供程序。 相反, 应用程序将使用[`FontsContract`](https://developer.android.com/reference/android/provider/FontsContract.html) API 的实例 ( [`FontsContractCompat`](https://developer.android.com/reference/android/support/v4/provider/FontsContractCompat.html)如果使用的是支持库 26)。  

Android 8.0 支持通过两种不同的方式下载字体:

1. **将可下载字体声明为资源**&ndash;应用可以通过 XML 资源文件向 Android 声明可下载字体。 这些文件将包含 Android 在应用程序启动并缓存到设备上时需要以异步方式下载字体的所有元数据。
2. **以编程方式**&ndash; Android 中的 api 级别26允许应用程序在应用程序运行时以编程方式下载字体。 应用程序将创建`FontRequest`给定字体的对象, 并将此对象传递`FontsContract`给类。 采用, 并从_字体提供程序_检索字体。 `FontRequest` `FontsContract` Android 将同步下载此字体。 本指南的后面部分`FontRequest`将显示创建的示例。
...
1. **fontProviderAuthority**&ndash;要用于请求的字体提供程序的颁发机构。
2. **fontPackage**&ndash;要用于请求的字体提供程序的包。 这用于验证提供程序的标识。
3. **fontQuery**&ndash;这是一个字符串, 可帮助字体提供程序查找请求的字体。 有关字体查询的详细信息特定于字体提供程序。 [`QueryBuilder`](https://github.com/xamarin/monodroid-samples/blob/master/android-o/DownloadableFonts/DownloadableFonts/QueryBuilder.cs) [可下载字体](https://github.com/xamarin/monodroid-samples/blob/master/android-o/DownloadableFonts/)示例应用中的类提供了一些有关 Google fonts 开源集合中字体的查询格式的信息。
...
可以通过实例化[`FontRequest`](https://developer.android.com/reference/android/support/v4/provider/FontRequest.html)对象并将`FontContractCompat.RequestFont`其传递给方法, 以编程方式下载字体。 此`FontContractCompat.RequestFont`方法将首先检查设备上是否存在该字体, 如果需要, 则会以异步方式查询该字体提供程序, 并尝试下载该应用的字体。 如果`FontRequest`无法下载字体, 则 Android 将使用默认系统字体。

`FontRequest`对象包含字体提供程序查找并下载字体时将使用的信息。 `FontRequest`需要四个信息片段:

1. **字体提供机构**&ndash;要用于请求的字体提供程序的颁发机构。
2. **字体包**&ndash;要用于请求的字体提供程序的包。 这用于验证提供程序的标识。
3. **字体查询**&ndash;这是一个字符串, 可帮助字体提供程序查找请求的字体。 有关字体查询的详细信息特定于字体提供程序。 字符串的详细信息特定于字体提供程序。 [`QueryBuilder`](https://github.com/xamarin/monodroid-samples/blob/master/android-o/DownloadableFonts/DownloadableFonts/QueryBuilder.cs) [可下载字体](https://github.com/xamarin/monodroid-samples/blob/master/android-o/DownloadableFonts/)示例应用中的类提供了一些有关 Google fonts 开源集合中字体的查询格式的信息。
4. **字体提供程序证书**&ndash;一个资源数组, 其中包含提供程序应使用其签名的证书的哈希集的列表。

此代码片段举例演示了如何实例化`FontRequest`新的对象:
...
在上一个代码`FontToDownload`段中, 是一种查询, 它将帮助 Google 字体开源集合中的字体。

在将传递`FontRequest` `FontContractCompat.RequestFont`给方法之前, 必须创建两个对象:

- **`FontsContractCompat.FontRequestCallback`** &ndash;这是必须扩展的抽象类。 当完成时`RequestFont` , 将调用该回调。 Xamarin Android 应用必须`FontsContractCompat.FontRequestCallback`对`OnTypefaceRequestFailed`和重写, 并`OnTypefaceRetrieved`提供在下载失败或全部成功时要执行的操作。
- **`Handler`** 如有必要, 将使用`RequestFont` 它在线程上下载字体。`Handler` &ndash; **不**应在 UI 线程上下载字体。

此代码段是一个C#类的示例, 该类将从 Google Fonts 开源集合中异步下载字体。 它实现`FontRequestCallback`接口, 并在`FontRequest`完成后C#引发事件。
...
若要使用此帮助器, `FontDownloadHelper`将创建一个新的, 并分配一个事件处理程序:  

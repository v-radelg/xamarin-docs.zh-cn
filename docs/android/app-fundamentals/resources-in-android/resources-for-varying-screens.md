...
从 API 级别 13 (Android 3.2) 开始, 屏幕大小将在使用 sw*N*dp 限定符时弃用。 此新限定符声明给定布局需要的空间量。 建议在 Android 3.2 或更高版本上运行的应用程序应使用这些较新的限定符。
...
对于面向最多为 12 (Android 3.1) 的 API 级别的应用程序, 这些布局应在使用限定符**小型**/**普通**/**大型**/**加大**的目录中, 作为大多数设备中提供的各种屏幕大小。 例如, 在下图中, 有四种不同屏幕大小的备用资源:
...
- **避免**[AbsoluteLayout](xref:Android.Widget.AbsoluteLayout)只要有可能&ndash; , 它会在 API 级别 3 (Android 1.5) 中弃用, 并将产生脆弱布局。  

...
让我们看一下如何使用`ShapeDrawable`创建2d 图形的快速示例。 `ShapeDrawable`可以定义四个基本形状之一：矩形、椭圆、直线和环。 还可以应用基本效果，如渐变、颜色和大小。 下面的 XML 是`ShapeDrawable`可在*AnimationsDemo*助理项目（文件`Resources/drawable/shape_rounded_blue_rect.xml`中）中找到的。
它定义带有紫色渐变背景和圆角的矩形：
...
可通过两种方式获取`Canvas`对象。 第一种方法涉及定义[位图](xref:Android.Graphics.Bitmap)对象，然后使用该对象`Canvas`实例化对象。 例如，以下代码片段创建了一个具有基础位图的新画布：
...
获取`Canvas`对象的另一种方法是提供[视图](xref:Android.Views.View)基类的[OnDraw](xref:Android.Views.View.OnDraw*)回调方法。 当该方法决定视图需要自行绘制并传入`Canvas`对象以使视图可以使用时，Android 会调用此方法。
...
- [Canvas.DrawText](xref:Android.Graphics.Canvas.DrawText*) &ndash; 用指定的颜色在画布上绘制文本。 在位置`x,y`绘制文本。
...
上面的代码首先创建一个红色的 paint 和一个绿色的 paint 对象。 它使用红色填充画布的内容，然后指示画布绘制一个绿色矩形，该矩形是画布宽度的 25%。 本文的源代码中包含的项目中`AnimationsDemo`显示了这种情况的一个示例。 通过启动应用程序并从主菜单中选择 "绘制项"，应显示类似于下面的屏幕：
...
可以通过因为这样做动画并将其应用到视图，以编程方式将动画应用于视图。 Android 提供帮助器类`Android.Views.Animations.AnimationUtils` ，它会使动画资源膨胀并返回的`Android.Views.Animations.Animation`实例。 此对象通过调用`StartAnimation`并`Animation`传递对象应用于视图。 下面的代码片段演示了一个示例：
...
但是，上面的代码片段并不是非常有用&ndash; ，animator 将运行，但不存在更新值的目标。 当`Animator`类确定需要向侦听器通知新值时，该类将引发 Update 事件。 应用程序可以提供事件处理程序来响应此事件，如以下代码片段所示：
...
现在我们已经了解`ValueAnimator`了，接下来了解更多`ObjectAnimator`相关信息。

#### <a name="using-the-objectanimator"></a>使用 ObjectAnimator

[ObjectAnimator](xref:Android.Animation.ObjectAnimator)是的一个子类`ViewAnimator` ，它将的`ValueAnimator`计时引擎和值计算与绑定事件处理程序所需的逻辑结合起来。 要求应用程序显式连接事件处理程序&ndash; `ObjectAnimator`将为我们执行此步骤。 `ValueAnimator`

的 api `ObjectAnimator`非常类似于的`ViewAnimator`api，但要求你提供对象以及要更新的属性的名称。 下面的示例演示使用`ObjectAnimator`的示例：
...
可绘制资源在一个 XML 文件中定义，该`<animation-list>`文件将元素作为根元素，并使用`<item>`一系列元素来定义动画中的每个帧。 此 XML 文件存储在应用程序`/Resource/drawable`的文件夹中。 以下 XML 是可绘制动画的示例：
...
- [使用动画将你的移动应用带入现实](http://youtu.be/ikSk_ILg3d0)

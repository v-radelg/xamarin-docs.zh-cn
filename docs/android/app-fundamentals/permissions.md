...
`ActivityCompat.ShouldShowRequestPermissionRationale`方法用于确定是否应向用户显示基本原理。 如果应显示给定`true`权限的基本原理, 则此方法将返回。 此屏幕截图显示了一个应用程序显示的 Snackbar 的示例, 该应用程序说明了应用程序需要知道设备位置的原因:
